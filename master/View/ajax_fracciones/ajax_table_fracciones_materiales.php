<?php 
	...
	// DB table to use
	$table = 'viewfraccionesmateriales';
		...
		array(
			...
			'formatter' => function ( $Status, $row ){
				...
				if ( $m['r'] == 1 ){
					...
					if ( $Status == 1 ){
						if ( $reglas->editar  ) $html.= '<li class="lista" id=""><a href="javascript:editar(\''.$row['id'].'\',\''.material.'\')" >Editar</li>';
						if ( $reglas->suspender 	) $html.= '<li><a href="javascript:suspender(\''.$row['id'].'\',\''.material.'\')">Suspender</li><div class=\"divider\"></div>';
						if ( $reglas->eliminar	) $html.= '<li><a href="javascript:eliminar(\''.$row['id'].'\',\''.material.'\')" >Eliminar</li>';
					}
					
					//// QUIERO COMENTARIOS AQUI
					else if ( $Status == 2 ){
						if ( $reglas->suspender ) $html.= '<li class="lista" id=""><a href="javascript:suspender(\''.$row['id'].'\',\''.material.'\')" >Reactivar</li><div class="divider"></div>';
						if ( $reglas->eliminar   ) $html.= '<li><a href="javascript:eliminar(\''.$row['id'].'\',\''.material.'\')" >Eliminar</li>';
					}
