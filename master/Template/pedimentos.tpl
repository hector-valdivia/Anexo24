...
<head>
	...
	<style type="text/css">
		...
		#partidas div.partida { padding-top: 20px; padding-bottom: 20px; }
		#partidas div.partida:nth-of-type(odd){ background-color: #E9F9FF; }
		#partidas div.partida table { width: 100%; }
		#partidas div.partida table > thead{ background-color: currentcolor; }
		#partidas div.partida table > thead tr{ color: #ffffff; }
		#partidas div.partida table > tbody tr td input{ margin-bottom:0px; }
		#partidas div.partida table > tfoot { background-color: #CDCDCD; }
		#partidas div.partida div:nth-child(1) { vertical-align: middle; text-align: center; }
	</style>
	<!-- END CSS TEMPLATE -->
</head>
		...
		<div class="row">
			<div class="col-md-12">
				...
				<div class="grid simple ">
					...
					<div class="grid-body">
						<!--------- =============================== Informacion general del pedimento =============================== -------->
							...
							<div class="row form-row partida">
								...
								<div class="col-md-11">
									...
									<div class="row form-row">
										...
										<div class="col-md-6">
											<label>Posible Numero de Parte</label>
											<input type="text" name="num_parte" class="form-control num_parte">
										</div>
									</div>
									<!-- Cuando esten bien puestos los numeros de parte descomentar esta area -->
									<!--
									<div class="row form-row">
										<div class="col-md-12">
		...
		$(document).ready(function(){
				...
				$('<div />', { 'class':'row form-row partida' }).append(
					...
					$('<div />', {  'class':'col-md-11' }).append(
						$('<div />', { 'class':'row form-row' }).append(
							...
							$('<div />', { 'class':'col-md-2' }).append(
								$('<label />',{ 'text':'UMC' }),
								$('<select />', { 'name':'umc', 'class':'form-control optUM umc' })
							),
							$('<div />', { 'class':'col-md-2' }).append(
								$('<label />',{ 'text':'CANT.UMC' }),
							...
							$('<div />', { 'class':'col-md-2' }).append(
								$('<label />',{ 'text':'UMT' }),
								$('<select />', { 'name':'umt', 'class':'form-control optUM umt' })
							),
								$('<div />', { 'class':'col-md-2' }).append(
								$('<label />',{ 'text':'CANT.UMT' }),
						...
						$('<div />', { 'class':'row form-row' }).append(
							...
							////Quitar cuando se coloquen bien los numeros de parte
							$('<div />', { 'class':'col-md-6' }).append(
									$('<label />',{ 'text':'Posible Numero de Parte' }),
									$('<input />', { 'type':'text', 'class':'form-control num_parte', 'name':'num_parte' })
							)
						),
						...
						/*$('<div />', { 'class':'row form-row' }).append(
							$('<div />', { 'class':'col-md-12' }).append(
								...
								$('<table />', { 'class':'table table-condensed table-responsive table-striped' }).append(
									$('<thead />').append(
										$('<tr />').append(
											$('<th />', { 'text':'Num. Parte' }),
											$('<th />', { 'text':'CANT.UMC' }),
			...
			$('#guardar').click(function ($e) {
				...
				if ( $('#pedimento').validationEngine('validate') ){
					...
					$data.facturas = [];
					///Get div proveedor
					var $proveedores = $('#proveedores').find('div.proveedor');
					...
					$data.partidas = [];
					///Get div partidas
					var $pp = $('#partidas').find('div.partida');
					$.each($pp,function(par,val){
						...
						$partida['sec'] = $(this).find('h1.sec').text();
						if ( !$.isEmpty( $partida.fraccion )  )
