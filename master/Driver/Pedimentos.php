<?php
/* Proyecto: Balance
 * Hecho por:Hector Manuel Valdivia Renteria
 * Modificado por: Hector Manuel Valdivia Renteria
 * Ultima fecha de modificacion: Viernes 13 de marzo del 2015
*/
...
class Pedimentos extends DB {
	...
	public function Pedimento($Data){
		try{
			...
			switch ( hacer ){
				case 'Agregar':
					$num_pedimento = Pedimentos::insertPedimento(pedimento);
					Pedimentos::insertFactura(facturas,$num_pedimento);
					Pedimentos::insertPartidas(partidas, $num_pedimento);
					//Pedimentos::insertPartidasNumerosParte(partidas_numparte, $num_pedimento);
				...
				case 'Editar':
					...
					//Pedimentos::deletePartidasNumerosParte($num_pedimento);
					//Pedimentos::insertPartidasNumerosParte(partidas_numparte, $num_pedimento);
	...
	public function countPedimento($num_pedimento){
		...
		Pedimentos::$DB->query("SELECT * FROM cat_anexo24_pedimento WHERE num_pedimento=:num_pedimento AND status IN(1,2)");
		Pedimentos::$DB->bind(':num_pedimento',$num_pedimento);
		Pedimentos::$DB->execute();
	...
	public function insertPartidas( $partidas, $num_pedimento ){
		...
		if ( is_array($partidas) && count($partidas) > 0 ){
			...
			foreach ( $partidas as $p ){
				if ( empty($p['descripcion']) ) $p['descripcion'] = 'No se escribio descripcion de la partida';
				Pedimentos::$DB->bind(':num_pedimento',$num_pedimento);
	...
	/// Insertar numperos de parte por partida
	public function insertPartidasNumerosParte($partidas_numparte, $num_pedimento){
		if ( empty($num_pedimento) ) throw new Exception("Error falta informacion para agregar partidas en numeros de parte",1);

		$partidas_numparte = unserialize($partidas_numparte);
		if ( is_array($partidas_numparte) && count($partidas_numparte) > 0 ){
			Pedimentos::$DB->query("INSERT INTO cat_anexo24_pedimento_partidas_numparte
				(num_pedimento, num_factura, num_parte, umc, umt)
				VALUES
				(:num_pedimento, :num_factura, :num_parte, :umc, :umt)
			");
			foreach( $partidas_numparte as $npp ){
				Pedimentos::$DB->bind(':num_pedimento',$npp['num_pedimento']);
				Pedimentos::$DB->bind(':num_factura',$npp['num_factura']);
				Pedimentos::$DB->bind(':num_parte',$npp['num_parte']);
				Pedimentos::$DB->bind(':umc',$npp['umc']);
				Pedimentos::$DB->bind(':umt',$npp['umt']);
				Pedimentos::$DB->execute();
			}
		}
	}
	///Delete numeros de parte por partida
	public function deletePartidasNumerosParte($num_pedimento){
		if ( empty($num_pedimento) ) throw new Exception("El numero de pedimento no puede estar vacio",1);
		Pedimentos::$DB->query("DELETE FROM cat_anexo24_pedimento_partidas_numparte WHERE num_pedimento=:num_pedimento");
		Pedimentos::$DB->bind(':num_pedimento',$num_pedimento);
		Pedimentos::$DB->execute();
	}
